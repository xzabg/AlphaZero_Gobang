...
class ChessClient():
    ...
    def get_room_info(self):
        ...
        action_result = json.loads(response.content)
        room = pickle.loads(bytes(action_result['info'], encoding="utf8"))
        return room
...
class GameStrategy_MZhang():
    def __init__(self, startplayer=0):
        ...
        policy_value_net = PolicyValueNet(self.height, self.width, model_file=model_file,output='output/')
        self.mcts_player = MCTSPlayer(policy_value_net.policy_value_fn, c_puct=1, n_playout=1000)
...
def go_play():
    ...
    parser.add_argument('--room_name', type=str, default='ayixin_1234')  # + str(time.time()))
    ...
    print("enter game success，you are: " + ("black" if user.game_role == 1 else "white"))
        ...
        if room.get_status() == GameRoom.ROOM_STATUS_NOONE or room.get_status() == GameRoom.ROOM_STATUS_ONEWAITING:
            ...
            if gameboard.get_current_user() == user.game_role:
                ...
                if action_result['id'] != 0:
                    print("go failed:")
                    # print
                    # ChessHelper.numToAlp(one_legal_piece[0]), ChessHelper.numToAlp(one_legal_piece[1])
            ...
            print("game over," + ("Black" if gameboard.get_winner() == 1 else "White") + " wins")
            break
